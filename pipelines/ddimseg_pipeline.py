    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps ) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # uncond_embeddings = uncond_embeddings.detach().clone().requires_grad_(False)
                cond_embeddings = prompt_embeds.detach().clone().requires_grad_(True)

                with torch.enable_grad():
                    opt_cond = torch.optim.Adam([cond_embeddings], lr=0.01 * (1. - i / 100.))
                    loop_number=200
                    indices_to_alter=[2,5]
                    
                    ### Target loss
                    alpha_cos=0.6
                    target_loss = 1.0 - ((0.9 - alpha_cos)/50*(50-i) + alpha_cos)
                    target_loss=0.05
                    ...
                    # predict the noise residual
                        ...
                        if not (cosine_loss < target_loss):
                            opt_cond.step()
                            print(print_string)       
                        else:
                            print(print_string)     
                ...
                cond_embeddings_list.append(cond_embeddings.detach().cpu())
        ...
        return all_latents, image, attention_maps_list, cond_embeddings_list
    ...
    def reconstruct(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # _prompt_embeds_ = cond_embeddings_list[i].cuda()
                _prompt_embeds_ = cond_embeddings_list[0].cuda()
                # predict the noise residual
                with torch.enable_grad():
                    noise_pred = self.unet(
                        latent_model_input,
        ...
        return image_rec, attention_maps_list
