...
from pipelines.det_null_textinv_pipeline import StableDiffusion_DetPipeline
...
def read_detfile(seg_image_path):
    seg_image = Image.open(seg_image_path).resize((16,16))
    seg_img_data = np.asarray(seg_image).astype(bool)
    if len(seg_img_data.shape) >2:
        seg_img_data=seg_img_data[:,:,-1]
...
if __name__=="__main__":
    ...
    postfix = f'cos_al_{args.alpha_cos}_beta_{args.beta_cos}_lam_{args.lam_cos}_'+ \
                f'iou_al_{args.alpha_iou}_beta_{args.beta_iou}_lam_{args.lam_iou}_'+ \
                    f'kl_al_{args.alpha_sim}_beta_{args.beta_sim}_lam_{args.lam_sim}_'+ \
    ...
    os.makedirs(os.path.join(args.results_folder, 
                             ...
                             exist_ok=True)
    os.makedirs(os.path.join(args.results_folder, 
                             f"null_inv_recon_DET/{postfix}"), 
                             exist_ok=True)
    os.makedirs(os.path.join(args.results_folder, 
                             f"embed_list_DET/{postfix}"), 
                             exist_ok=True)
    
    pipeline = StableDiffusion_DetPipeline.from_pretrained(
        sd_model_ckpt,
    ...
    for ind in range(len(seg_search_words)):
        ...
        if os.path.isfile(os.path.join(args.seg_dirs, f"{object_name}.png")):
            ...
            det_image_path = os.path.join(args.seg_dirs, f"{object_name}_bbox.png")
        else:
            seg_image_path = os.path.join(args.seg_dirs, f"{object_name}.jpg")
            det_image_path = os.path.join(args.seg_dirs, f"{object_name}_bbox.jpg")

        seg_maps_paths.append(seg_image_path)

        print(f'read segmentation and detection maps from {det_image_path}')
        seg_maps.append(read_segfile(seg_image_path))
        det_maps.append(read_detfile(det_image_path))
    ...
    with open(os.path.join(args.results_folder, 
            f"embed_list_DET/{postfix}/{bname}_uncond.pkl"), 
            'wb') as f:
        pkl.dump(uncond_embeddings_list, f)

    with open(os.path.join(args.results_folder, 
            f"embed_list_DET/{postfix}/{bname}_cond.pkl"), 
            'wb') as f:
    ...
    rec_pil[0].save(os.path.join(args.results_folder, 
            f"null_inv_recon_DET/{postfix}/{bname}.png"))
    
    with open(os.path.join(args.results_folder, 
            f"attn_DET/{postfix}/{bname}.pkl"), 
            'wb') as f:
    ...
    single_attn_paths = save_attn_avg(save_path=os.path.join(args.results_folder, f"attn_DET/{postfix}"), 
    ...
    IoU_txt_file = os.path.join(args.results_folder, f"attn_DET/{postfix}", 'iou.txt')
    IoUs=[]
